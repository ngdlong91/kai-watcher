...
func (n *node) getUDBEntries(ctx context.Context, valSmcAddr common.Address, delegatorAddr common.Address) (*big.Int, *big.Int, error) {
	payload, err := n.validatorSMC.Abi.Pack("getUBDEntries", delegatorAddr)
	if err != nil {
		n.lgr.Error("Error packing UDB entry payload: ", zap.Error(err))
		return nil, nil, err
	}
	res, err := n.KardiaCall(ctx, constructCallArgs(valSmcAddr.Hex(), payload))
	if err != nil {
